...
- **핀 카드는 상단 고정 섹션에도 나오고, 원래 카테고리 블록에도 그대로 나온다** (양쪽 노출, 원래 자리 유지).
...
- `PersonalBoardView.tsx`
  ...
  - 중복 렌더 대비: 핀 섹션 카드의 dnd/React key는 `pin:${card.key}`로 접두사 부여.
...
- 없음(합의 완료). 헤더는 카테고리 이름만(개수 없음), 핀 카드는 양쪽 노출, 핀 버튼은 편집 모드 전용.
