...
- Create: `src/components/SortableCard.tsx` (dnd-kit 래퍼)
  ...
  return (
    <main className={styles.page}>
      ...
      <div className={styles.catRow}>
        ...
        {categories.map((c) => (
          <span key={c.id} className={styles.catInput}>
            {c.name}
            <button onClick={() => removeCategory(c.id)}
              style={{ border: "none", background: "transparent", color: "var(--text-dim)", marginLeft: 6 }}>✕</button>
          </span>
        ))}
      </div>
...
- **카테고리 정렬:** `reorderCategories`는 데이터 레이어/API에 존재(Task 6,7). UI 노출은 1차에선 생략(카드 정렬 우선), 카테고리 순서변경 UI는 이후 추가 가능 — 스펙의 "카테고리 순서 드래그"는 백엔드까지 준비되어 있고 UI만 후속.
